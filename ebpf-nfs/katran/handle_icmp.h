...
static inline int parse_icmpv6(void *data, void *data_end, __u64 off,
  ...
  if(icmp_hdr->icmp6_type == ICMPV6_PKT_TOOBIG)
    VIGOR_TAG(TRAFFIC_CLASS,"ICMPV6_PKT_TOOBIG");
  else if(icmp_hdr->icmp6_type == ICMPV6_DEST_UNREACH)
    VIGOR_TAG(TRAFFIC_CLASS, "ICMPV6_DEST_UNREACH");
  else 
    return XDP_PASS;
...
static inline int parse_icmp(void *data, void *data_end, __u64 off,
  ...
  if (icmp_hdr->type == ICMP_ECHO) {
    VIGOR_TAG(TRAFFIC_CLASS, ICMP_ECHO);
